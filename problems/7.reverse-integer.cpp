 ...
 * [7] Reverse Integer
 * 
 * // Note: lesson learnt is that, leetcode problems don't usually involve
...
public:
    int reverse(int x) {
        ...
        while (x) {
            // Don't need to worry about the leading digit causing overflow,
            // since the first digit is <= 2 and last digit is 7 > 2.
            if (output > numeric_limits<int>::max() / 10 || output < numeric_limits<int>::min() / 10) {
                return 0;
            }
